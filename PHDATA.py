...
airports_df.write.format("snowflake").options(**options).option("dbtable","airlines").mode("overwrite").save()
flights_df.write.format("snowflake").options(**options).option("dbtable","airlines").mode("overwrite").save()
...
# MAGIC select
# MAGIC   f.AIRPORT,
# MAGIC   collect_list(f.listdelay) as Delay_reasons
# MAGIC from
# MAGIC   query2 f
# MAGIC group by
# MAGIC   f.AIRPORT
...
select
  f.AIRPORT,
  collect_list(f.listdelay) as Delay_reasons
from
  query2 f
group by
  f.AIRPORT"""
